...
#define MPP_BIT_NUM(MppNum) 	(MppNum%32)
#define MPP_CTRL_REG(MppNum) 	(0x10000 + 0x04*(MppNum/8))
#define GPP_DATA_OUT_EN_REG(grp)                ((grp == 0) ? 0x10104 : 0x10144)
#define GPP_BLINK_EN_REG(grp)                   ((grp == 0) ? 0x10108 : 0x10148)
#define GPP_REG_BIT_SET(RegNum, BitNum) 	MV_REG_WRITE(RegNum, ((1<<BitNum)|MV_REG_READ(RegNum)))
#define GPP_REG_BIT_CLR(RegNum, BitNum) 	MV_REG_WRITE(RegNum, (~(1<<BitNum)&MV_REG_READ(RegNum)))

int lg_bootmode=0;

void lg_Hdd_Led_Ctrl(int mode)
{
    const int Hdd_MppNum=35;

    if(mode == ENC_LED_BLINK) {
        // 1. change to GPIO mode, HDD=MPP35
        MV_REG_WRITE(MPP_CTRL_REG(Hdd_MppNum), (~(0xF<<((Hdd_MppNum%4)*4)))&MV_REG_READ(MPP_CTRL_REG(Hdd_MppNum)));
        // 2. make output enable register to disable mode
        GPP_REG_BIT_CLR(GPP_DATA_OUT_EN_REG(Hdd_MppNum/32), MPP_BIT_NUM(Hdd_MppNum));
        // 3. set up blink mode
        GPP_REG_BIT_SET(GPP_BLINK_EN_REG(Hdd_MppNum/32), MPP_BIT_NUM(Hdd_MppNum));
    } else if (mode == ENC_LED_ON) {
        // 1. change to GPIO mode, HDD=MPP35
        MV_REG_WRITE(MPP_CTRL_REG(Hdd_MppNum), (~(0xF<<((Hdd_MppNum%4)*4)))&MV_REG_READ(MPP_CTRL_REG(Hdd_MppNum)));
        // 2. make output enable register to enable mode
        GPP_REG_BIT_CLR(GPP_DATA_OUT_EN_REG(Hdd_MppNum/32), MPP_BIT_NUM(Hdd_MppNum));
        // 3. clear blink register
        GPP_REG_BIT_CLR(GPP_BLINK_EN_REG(Hdd_MppNum/32), MPP_BIT_NUM(Hdd_MppNum));
        // 4. clear output register
        GPP_REG_BIT_SET(GPP_DATA_OUT_REG(Hdd_MppNum/32), MPP_BIT_NUM(Hdd_MppNum));
    } else if (mode == ENC_LED_OFF) {
        // 1. change to GPIO mode, HDD=MPP35
        MV_REG_WRITE(MPP_CTRL_REG(Hdd_MppNum), (~(0xF<<((Hdd_MppNum%4)*4)))&MV_REG_READ(MPP_CTRL_REG(Hdd_MppNum)));
        // 2. make output enable register to enable mode
        GPP_REG_BIT_CLR(GPP_DATA_OUT_EN_REG(Hdd_MppNum/32), MPP_BIT_NUM(Hdd_MppNum));
        // 3. clear blink register
        GPP_REG_BIT_CLR(GPP_BLINK_EN_REG(Hdd_MppNum/32), MPP_BIT_NUM(Hdd_MppNum));
        // 4. set output register
        GPP_REG_BIT_CLR(GPP_DATA_OUT_REG(Hdd_MppNum/32), MPP_BIT_NUM(Hdd_MppNum));
    } else {
        printf("lg_Hdd_Led_Ctrl has an abnomal mode parameter!!! \n");
    }
}

void lg_Odd_Led_Ctrl(int mode)
{
    const int Odd_MppNum=34;

    if(mode == ENC_LED_BLINK) {
        // 1. change to GPIO mode, HDD=MPP35
        MV_REG_WRITE(MPP_CTRL_REG(Odd_MppNum), (~(0xF<<((Odd_MppNum%4)*4)))&MV_REG_READ(MPP_CTRL_REG(Odd_MppNum)));
        // 2. make output enable register to disable mode
        GPP_REG_BIT_CLR(GPP_DATA_OUT_EN_REG(Odd_MppNum/32), MPP_BIT_NUM(Odd_MppNum));
        // 3. set up blink mode
        GPP_REG_BIT_SET(GPP_BLINK_EN_REG(Odd_MppNum/32), MPP_BIT_NUM(Odd_MppNum));
    } else if (mode == ENC_LED_ON) {
        // 1. change to GPIO mode, HDD=MPP35
        MV_REG_WRITE(MPP_CTRL_REG(Odd_MppNum), (~(0xF<<((Odd_MppNum%4)*4)))&MV_REG_READ(MPP_CTRL_REG(Odd_MppNum)));
        // 2. make output enable register to enable mode
        GPP_REG_BIT_CLR(GPP_DATA_OUT_EN_REG(Odd_MppNum/32), MPP_BIT_NUM(Odd_MppNum));
        // 3. clear blink register
        GPP_REG_BIT_CLR(GPP_BLINK_EN_REG(Odd_MppNum/32), MPP_BIT_NUM(Odd_MppNum));
        // 4. clear output register
        GPP_REG_BIT_SET(GPP_DATA_OUT_REG(Odd_MppNum/32), MPP_BIT_NUM(Odd_MppNum));
    } else if (mode == ENC_LED_OFF) {
        // 1. change to GPIO mode, HDD=MPP35
        MV_REG_WRITE(MPP_CTRL_REG(Odd_MppNum), (~(0xF<<((Odd_MppNum%4)*4)))&MV_REG_READ(MPP_CTRL_REG(Odd_MppNum)));
        // 2. make output enable register to enable mode
        ...
        GPP_REG_BIT_CLR(GPP_BLINK_EN_REG(Odd_MppNum/32), MPP_BIT_NUM(Odd_MppNum));
        // 4. set output register
        GPP_REG_BIT_CLR(GPP_DATA_OUT_REG(Odd_MppNum/32), MPP_BIT_NUM(Odd_MppNum));
    } else {
        printf("lg_Odd_Led_Ctrl has an abnomal mode parameter!!! \n");
    }

}
void lg_Usb_Led_Ctrl(int mode)
{
    const int Usb_MppNum=30;

    if(mode == ENC_LED_BLINK) {
        // 1. change to GPIO mode, HDD=MPP30
        MV_REG_WRITE(MPP_CTRL_REG(Usb_MppNum), (~(0xF<<((Usb_MppNum%4)*4)))&MV_REG_READ(MPP_CTRL_REG(Usb_MppNum)));
        // 2. make output enable register to disable mode
        GPP_REG_BIT_CLR(GPP_DATA_OUT_EN_REG(Usb_MppNum/32), MPP_BIT_NUM(Usb_MppNum));
        // 3. set up blink mode
        GPP_REG_BIT_SET(GPP_BLINK_EN_REG(Usb_MppNum/32), MPP_BIT_NUM(Usb_MppNum));
    } else if (mode == ENC_LED_ON) {
        // 1. change to GPIO mode, HDD=MPP35
        MV_REG_WRITE(MPP_CTRL_REG(Usb_MppNum), (~(0xF<<((Usb_MppNum%4)*4)))&MV_REG_READ(MPP_CTRL_REG(Usb_MppNum)));
        // 2. make output enable register to enable mode
        GPP_REG_BIT_CLR(GPP_DATA_OUT_EN_REG(Usb_MppNum/32), MPP_BIT_NUM(Usb_MppNum));
        // 3. clear blink register
        GPP_REG_BIT_CLR(GPP_BLINK_EN_REG(Usb_MppNum/32), MPP_BIT_NUM(Usb_MppNum));
        // 4. clear output register
        GPP_REG_BIT_SET(GPP_DATA_OUT_REG(Usb_MppNum/32), MPP_BIT_NUM(Usb_MppNum));
    } else if (mode == ENC_LED_OFF) {
        // 1. change to GPIO mode, HDD=MPP35
        MV_REG_WRITE(MPP_CTRL_REG(Usb_MppNum), (~(0xF<<((Usb_MppNum%4)*4)))&MV_REG_READ(MPP_CTRL_REG(Usb_MppNum)));
        // 2. make output enable register to enable mode
        ...
        GPP_REG_BIT_CLR(GPP_BLINK_EN_REG(Usb_MppNum/32), MPP_BIT_NUM(Usb_MppNum));
        // 4. set output register
        GPP_REG_BIT_CLR(GPP_DATA_OUT_REG(Usb_MppNum/32), MPP_BIT_NUM(Usb_MppNum));
    } else {
        printf("lg_Usb_Led_Ctrl has an abnomal mode parameter!!! \n");
    }
...
////////  >>> enclosure
    int Button_L = 31;
        ...
        if ((MV_REG_READ(GPP_DATA_IN_REG(Button_L/32)) & (1 << Button_L%32)) == 0) {
            lg_bootmode = 1; // enclosure mode
            printf("backup button is pushed, enclosure is begun! \n");
            MV_REG_WRITE(GPP_DATA_IN_REG(0),(1<<30)|MV_REG_READ(GPP_DATA_IN_REG(0)));
            lg_Usb_Led_Ctrl(ENC_LED_BLINK);
            lg_Odd_Led_Ctrl(ENC_LED_BLINK);
            lg_Hdd_Led_Ctrl(ENC_LED_BLINK);
        } else {
            printf("Normal mode boot start! \n");
        }
////////  <<< enclosure
...
void print_dev_id(void){
	...
	printf("\n ** LGE-NAS BOARD: %s BE ",boardName);
#else
	printf("\n ** LGE-NAS BOARD: %s LE ",boardName);
#endif
    return;
}
...
void misc_init_r_env(void){
	...
	{
		sprintf(buff,"console=ttyS0,115200 mtdparts=spi_flash:0x%x@0(uboot),0x%x@0x%x(env)",
			 CFG_MONITOR_LEN, CFG_ENV_SIZE, CFG_ENV_ADDR - CFG_MONITOR_BASE);
		//sprintf(buff,"console=ttyS0,115200 mtdparts=spi_flash:0x%x@0(uboot)ro,0x%x@0x%x(root)",
		//	 ubootSize, flashSize - 0x100000, 0x100000);
		env = getenv("console");
		if(!env || strcmp(buff, env))
			setenv("console",buff);
	}
...
//////////  >>>	
        ...
        env = getenv("boot_name");
        if(!env)
                setenv("boot_name","u-boot.bin");
 
        env = getenv("up-boot");
        if(!env)
                setenv("up-boot","tftp 2000000 nt/$(boot_name); sflash protect off; sflash erase all; sflash write 2000000 0 $(filesize)");

        env = getenv("root_path");
        if(!env)
                setenv("root_path","root=/dev/md0 rw");

        env = getenv("read-kern");
        if(!env)
		setenv("read-kern","ide reset; ext2load ide 0 2000000 /boot/$(image_name);");

        env = getenv("mem-clear");
        if(!env)
		setenv("mem-clear","mw.l 1000000 0x00000000 1000000");

        env = getenv("bootcmd_nfs");
        if(!env)
		setenv("bootcmd_nfs","tftpboot 0x2000000 $(image_name); \
...
ip=$(ipaddr):$(serverip)$(bootargs_end) $(mvNetConfig) $(mvPhoneConfig);  bootm 0x2000000; ");
        ...
        env = getenv("bootcmd_hdd");
        if(!env)
		setenv("bootcmd_hdd","run read-kern; setenv bootargs $(console) $(root_path) lpj=4964352 nas_type=$(nas_type); bootm 0x2000000; ");

        if( !getenv("bootargs_usb") )
                setenv("bootargs_usb", "root=/dev/sdc2 rw rootdelay=10");

        if( !getenv("bootcmd_enc") )
                setenv("bootcmd_enc", "run mem-clear; set bootargs $(console) $(bootargs_ram) nas_type=$(nas_type) enclosure; ide res; ide load; bootm 1000000 2000000");

        if( !getenv("bootargs_ram") )
                setenv("bootargs_ram", "root=/dev/ram0 rw");

        if( !getenv("tftp-ram") )
                setenv("tftp-ram", "tftp 1000000 nt/$(uImage-ram); tftp 2000000 nt/ramdisk_image; set bootargs $(console) $(bootargs_ram); bootm 1000000 2000000");

        if( !getenv("uImage-ram") )
                setenv("uImage-ram", "uImage-2.6.30-nt1_11_ramdisk");

        if( !getenv("bootcmd") )
                setenv("bootcmd", "run bootcmd_hdd");

        if( !getenv("bootcmd_tftp") )
                setenv("bootcmd_tftp", "tftp 0x2000000 $(image_name); setenv bootargs $(console) $(root_path) lpj=4964352; bootm 0x2000000; ");
		if( !getenv("firmware") ) 
                setenv("firmware", "firmware.tar.gz");

        if( !getenv("bootcmd_netenc") )
                setenv("bootcmd_netenc", "set bootargs $(console) $(bootargs_ram) netenc nas_type=$(nas_type) IP=$(ipaddr) SIP=$(serverip) FW=$(firmware); bootm 1000000 2000000");
...
ip=$(ipaddr):$(serverip)$(bootargs_end);  bootm 0x2000000; ");
        ...
        if(!env)
#if defined(MV_INCLUDE_TDM) && defined(MV_INC_BOARD_QD_SWITCH)
	    setenv("standalone","ide reset; ext2load ide 0 0x2000000 /boot/$(image_name);setenv bootargs $(console) \
$(root_path) ip=$(ipaddr):$(serverip)$(bootargs_end) $(mvNetConfig) $(mvPhoneConfig); bootm 0x2000000;");
#elif defined(MV_INC_BOARD_QD_SWITCH)
            setenv("standalone","ide reset; ext2load ide 0 0x2000000 /boot/$(image_name);setenv bootargs $(console) \
$(root_path) ip=$(ipaddr):$(serverip)$(bootargs_end) $(mvNetConfig); bootm 0x2000000;");
#elif defined(MV_INCLUDE_TDM)
            setenv("standalone","ide reset; ext2load ide 0 0x2000000 /boot/$(image_name);setenv bootargs $(console) \
$(root_path) ip=$(ipaddr):$(serverip)$(bootargs_end) $(mvPhoneConfig); bootm 0x2000000;");
#else
            setenv("standalone","ide reset; ext2load ide 0 0x2000000 /boot/$(image_name);setenv bootargs $(console) \
$(root_path) ip=$(ipaddr):$(serverip)$(bootargs_end); bootm 0x2000000;");
#endif
	#endif
...
*
*******************************************************************************/
void hdd_power_restart(void)
{
	int s = 0;
	int hdd_on_mpp = 15;

	printf("HDD Power Restart (MPP Bit:%d)\n", hdd_on_mpp);

	if( !getenv("hdd_off_delay")) {
		setenv("hdd_off_delay", "4000000");
		saveenv();
	}
	s = (int) simple_strtoul (getenv("hdd_off_delay"), NULL, 10);

	GPP_REG_BIT_CLR(GPP_DATA_OUT_EN_REG(hdd_on_mpp/32), MPP_BIT_NUM(hdd_on_mpp));
	GPP_REG_BIT_CLR(GPP_DATA_OUT_REG(hdd_on_mpp/32), MPP_BIT_NUM(hdd_on_mpp));
	udelay(s);

	GPP_REG_BIT_CLR(GPP_DATA_OUT_EN_REG(hdd_on_mpp/32), MPP_BIT_NUM(hdd_on_mpp));
	GPP_REG_BIT_SET(GPP_DATA_OUT_REG(hdd_on_mpp/32), MPP_BIT_NUM(hdd_on_mpp));
	udelay(1000);
}
...
static void mvHddPowerCtrl(void)
{
	...
	MV_32 fanHigh = 0;
	char* env;
	MV_32 board_id =  mvBoardIdGet();

    if(RD_88F6281A_ID == board_id)
    {
...
*/
	...
	if ((RD_88F6281A_ID == board_id) || (RD_88F6192A_ID == board_id) || 
        (RD_88F6190A_ID == board_id))
	{
...
void recoveryHandle(void)
{
    ...
    sprintf(cmd,"setenv bootargs $(console) root=/dev/ram0 rootfstype=squashfs initrd=0x%x,0x%x ramdisk_size=%d recovery=%s serverip=%d.%d.%d.%d  ethact=$(ethact) ethaddr=$(ethaddr) eth1addr=$(eth1addr); bootm 0x%x;", 
            imagAddr + 0x200000, (imagSize - 0x300000), (imagSize - 0x300000)/1024, ip, (NetServerIP & 0xFF), ((NetServerIP >> 8) & 0xFF), ((NetServerIP >> 16) & 0xFF), ((NetServerIP >> 24) & 0xFF),imagAddr);
    ...
    {
        setenv("console","console=ttyS0,115200 mtdparts=spi_flash:0x100000@0x0(uboot),0x200000@0x100000(uimage),0xb80000@0x300000(rootfs),0x180000@0xe80000(varfs),0xf00000@0x100000(flash) varfs=/dev/mtdblock3");
