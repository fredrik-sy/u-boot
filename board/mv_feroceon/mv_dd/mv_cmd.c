...
int resetenv_cmd(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	if (whoAmI())
	{
	    puts ("Only Master CPU can resetenv!\n");
	    return 0;
	}
...
{
	mvSemaLock(simple_strtoul( argv[1], NULL, 16 ));
	return 1;
}

U_BOOT_CMD(
	lockSem,	2,	0,	do_lock,
	"lockSem\t- Lock semaphore <number>\n",
	"Wait in busy loop until succeed\n"
);

int do_unlock (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	mvSemaUnlock(simple_strtoul( argv[1], NULL, 16 ));
	return 1;
}

U_BOOT_CMD(
	unlockSem,	2,	0,	do_unlock,
	"unlockSem\t- Unlock semaphore <number>\n",
	"\n"
);
#endif	/* CFG_CMD_RCVR */
