...
////////// >>> for Enclosure setup
        ...
        if (lg_bootmode == 1) { //enclosure
                printf(" ENCLOSURE mode : lg_bootmode = %d \n" , lg_bootmode);
				unsigned int nas_type = 0;
				char * s;

				 if ((s = getenv ("nas_type")) != NULL) {
					 nas_type = simple_strtoul (s, NULL, 16);
				 }

				if( nas_type & 0x00000001 == 0x00000001 ) {
					printf("Starting Network Enclosing...\n");
					nasd_main();
				}
				else {
					printf("Starting ODD Enclosing...\n");
					sprintf(tmp_s, "run bootcmd_enc");
					run_command(tmp_s,0);
				}
        } else {
                printf(" Normal mode : lg_bootmode = %d \n" , lg_bootmode);
        }
////////// <<<
